...
//     let request;
//     if (authMode === 'login') {
//       request = axios.post('http://localhost:5173/login', authData);
//     } else {
//       request = axios.post('http://localhost:5173/signup', authData);
//     }
//     request
//       .then(authResponse => {
...
export const App: React.FC = () => {
  ...
  const authHandler = (event: React.FormEvent, authData: { email: string; password: string }) => {
    event.preventDefault();
    if (authData.email.trim() === '' || authData.password.trim() === '') {
      return;
    }
    const request = authMode === 'login'
      ? axios.post('http://localhost:3100/login', authData)
      : axios.post('http://localhost:3100/signup', authData);
  ...
  const renderRoutes = () => {
    if (!isAuth) {
      return (
        <Routes>
          ...
          <Route path="/auth" element={<AuthPage mode={authMode} onAuth={authHandler} onAuthModeChange={authModeChangedHandler} />} />
        </Routes>
      );
    }
    ...
    return (
      <Routes>
        ...
        <Route path="/auth" element={<Navigate to="/products" replace />} />
        <Route path="/signup" element={<Navigate to="/products" replace />} />
        <Route path="/product/:mode" element={<ProductEditPage onError={errorHandler} />} />
        <Route path="/products/:id/:mode" element={<ProductEditPage onError={errorHandler} />} />
        <Route path="//products/:mode" element={<ProductAddPage onError={errorHandler} />} />
        <Route path="/products/:id" element={<ProductDetailsPage onError={errorHandler} />} />
        <Route path="/products" element={<ProductsPage onError={errorHandler} />} />
      </Routes>
    );
  };
